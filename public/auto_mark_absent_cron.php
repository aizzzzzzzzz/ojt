 ...
 * Runs daily at 7:00 PM to mark students as absent if:
...
if (!$is_past_7pm && !$force_run) {
    if ($is_cli) {
        echo "Not yet 7:00 PM. Current time: " . $now->format('H:i:s') . "\n";
...
try {
    ...
    foreach ($students as $student) {
        ...
        if (!$attendance) {
            ...
            audit_log($pdo, 'Auto Mark Absent', "Student ID $student_id ($student_name) - no attendance by 7PM");
        } else {
            // Attendance exists
            if ($attendance['status'] === 'Absent') {
                ...
                audit_log($pdo, 'Auto Mark Absent', "Student ID $student_id ($student_name) - unverified attendance changed to absent");
