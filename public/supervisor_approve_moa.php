        ...
        try {
            if ($action === 'approve') {
                ...
                if (empty($rejection_reason)) {
                    ...
                    $success_message = "Document rejected. Student will be notified.";
                }
            }
...
$pending_stmt = $pdo->prepare("
    ...
    WHERE m.supervisor_approval_status = 'pending'
...
$history_stmt = $pdo->prepare("
    ...
    FROM moa_documents m
    JOIN students s ON m.student_id = s.student_id
    WHERE m.supervisor_approval_status IN ('approved', 'rejected')
    ORDER BY COALESCE(m.supervisor_approved_at, m.uploaded_at) DESC
    LIMIT 20
");
...
<head>
    ...
    <title>Approve MOA Documents</title>
...
<div class="page-container">
    <div class="page-header">
        <div>
            <h1>📋 Approve MOA Documents</h1>
            <p>Review and approve student documents</p>
        </div>
        <a href="supervisor_dashboard.php" class="back-btn">⬅ Back</a>
    </div>
    ...
    <div class="section">
        <h2>⏳ Pending Approval (<?= count($pending_documents) ?>)</h2>
        
        <?php if (empty($pending_documents)): ?>
            <div class="no-items">
                <p>No documents pending approval. All caught up! ✓</p>
            </div>
        <?php else: ?>
                        ...
                        <div class="rejection-form">
                            <form method="POST">
                                ...
                                <div style="margin-top: 8px; display: flex; gap: 8px;">
                                    ...
                                    <button type="button" onclick="toggleRejectForm(this.classList.contains('close') ? this.parentElement.parentElement.previousElementSibling : null)" class="btn btn-secondary" style="flex: 1; cursor: pointer;">Cancel</button>
                                </div>
                            </form>
                        </div>
                    </div>
            ...
            <?php foreach ($history_documents as $doc): ?>
                <div class="document-card" style="opacity: 0.8;">
                    <div class="document-info">
                        ...
                        <?php if ($doc['supervisor_approval_status'] === 'approved'): ?>
                            <p>
                                <strong>Approved on:</strong> <?= date('M d, Y \a\t H:i', strtotime($doc['supervisor_approved_at'])) ?>
                            </p>
                        <?php elseif ($doc['supervisor_approval_status'] === 'rejected'): ?>
                            <p style="color: #b91c1c;">
                                <strong>Rejected</strong>
                                <?php if ($doc['supervisor_rejection_reason']): ?>
                                    <br><em><?= htmlspecialchars($doc['supervisor_rejection_reason']) ?></em>
                                <?php endif; ?>
                            </p>
                        <?php endif; ?>
                        <p style="font-size: 12px; margin-top: 8px;">
                            Admin Status: 
                            <span class="status-badge" style="background: #f0f9ff; color: #0369a1;">
                                <?= ucfirst($doc['admin_approval_status']) ?>
                            </span>
                        </p>
                    </div>
